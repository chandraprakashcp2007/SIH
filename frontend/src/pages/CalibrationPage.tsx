...
export const CalibrationPage: React.FC = () => {
  ...
  return <section className="p-4 sm:p-6 max-w-6xl mx-auto" aria-labelledby="calibration-title">
    <header className="mb-5"><h1 id="calibration-title" className="text-xl font-bold flex items-center gap-2"><Gauge className="text-accent-info"/>Sensor Calibration</h1><p className="text-xs text-text-muted mt-1">Persisted offsets and baselines. Changes require administrator access.</p></header>
    ...
    <div className="grid lg:grid-cols-3 gap-4">{Object.entries(data.nodes).map(([node, values]: any) => <article key={node} className="border border-border-subtle bg-bg-secondary">
      ...
      <div className="p-4 space-y-3">{Object.entries(values).map(([key, value]: any) => <label key={key} className="block text-[11px] text-text-secondary"><span className="block mb-1">{key.replaceAll('_',' ')}</span><input aria-label={`${node} ${key}`} type="number" step="any" value={value} onChange={(e) => update(node,key,e.target.value)} className="w-full bg-bg-surface border border-border-subtle px-3 py-2 font-mono text-xs focus:outline-none focus:border-accent-info"/></label>)}</div>
      <button onClick={() => save(node)} disabled={saving === node} className="m-4 mt-0 px-3 py-2 bg-accent-info text-bg-primary text-xs font-bold flex items-center gap-2 disabled:opacity-50">{saving === node ? <RefreshCw className="w-4 h-4 animate-spin"/> : <Save className="w-4 h-4"/>}Save calibration</button>
    </article>)}</div>
  </section>;
};
